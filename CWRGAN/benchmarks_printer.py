...
class BenchmarkPrinter(object):
    ...
    def print(cls, data, widths, fmts=None, col_sep='|', upper='', lower=''):
        ...
        formatted_data = [fmt.format(d) for fmt, d in zip(fmts, data)]
        columns = [('{: ^%i}' % w).format(fd) for w, fd in zip(widths, formatted_data)]
    ...
    def print_benchnames(self):
        BenchmarkPrinter.print(self.columns, self.widths, upper='_', lower='-')
